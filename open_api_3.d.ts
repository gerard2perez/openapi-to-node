...
export interface IComponentsObject {
   ...
   */
   responses: { [name: string]: IResponseObject };

  /**
   * An object to hold reusable [Parameter Objects](#parameterObject).
   */
   parameters: { [name: string]: IParameterObject };
   ...
   */
   requestBodies: { [name: string]: IRequestBodyObject };
   ...
   */
   securitySchemes: { [name: string]: ISecuritySchemeObject };
   ...
   */
   callbacks: { [name: string]: ICallbackObject };
}
 ...
 * `/itemsid=###`, the query parameter is `id`.
...
export interface IResponsesObject {
   ...
   * an object. Can have any valid JSON format value.
   */
  [httpStatusCodeHttpCodesAndExtension: string]: IResponseObject | any;
 ...
 * @example:
```http
  POST /subscribe/myeventqueryUrl=http://clientdomain.com/stillrunning HTTP/1.1
  Host: example.org
...
```yaml
  myWebhook:
    'http://notificationServer.comtransactionId={$request.body#/id}&email={$request.body#/email}':
      post:
        requestBody:
